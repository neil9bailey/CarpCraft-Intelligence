...
class CarpCraftApiClient {
  ...
  Future<List<dynamic>> getList(String path) async {
    ...
    throw const CarpCraftApiException('Expected a list response from CarpCraft API.');
  }
  ...
  Future<Map<String, dynamic>> postMap(String path, Map<String, dynamic> body) async {
    ...
    throw const CarpCraftApiException('Expected an object response from CarpCraft API.');
  }
  ...
  Future<dynamic> _send(String method, String path, {Map<String, dynamic>? body}) async {
    ...
    try {
      final request = await client.openUrl(method, Uri.parse('$baseUrl$path')).timeout(timeout);
      ...
      final payload = await response.transform(utf8.decoder).join().timeout(timeout);
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw CarpCraftApiException('CarpCraft API returned ${response.statusCode}: $payload');
      }
